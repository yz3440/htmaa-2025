...
I'm most comfortable designing in Figma, but designing for print isn't a native feature. My workaround (shamefully hacky) is to assign each pixel a real physical unit — in this design I'm using 1px = 1mm. So I have a 2794×4318 canvas and each film block is 360 × 350 (36mm × 35mm). When I export, I use 4× resolution to keep the text sharp, and when I print, I set the paper to 11×17 and scale to cover 100%. Not elegant, but it works.
...
```cpp
// L-mark detection kernel scoring
inline float scoreLMark(const GrayImage& img, int cx, int cy, LMarkType type,
    ...
    // Measure horizontal and vertical arm lengths
    int hLen = 0, vLen = 0;
    for (int i = 1; i <= maxArm; i++) {
        if (img.get(cx + hDir * i, cy) < darkThresh) hLen++;
        else break;
    }
    for (int i = 1; i <= maxArm; i++) {
        if (img.get(cx, cy + vDir * i) < darkThresh) vLen++;
        else break;
    }
...
I built a web interface served directly from the ESP32S3 to preview the camera feed and test the pipeline in real-time. The interface includes orientation controls (8 rotation/flip combinations) and a keystone correction slider:

```cpp
// Web debug interface - handles processed image with transforms
void handleProcessed() {
  ...
  // Step 1: 90 degree rotation (camera is mounted sideways)
  bool ok = (orientMode < 4) ? rotate90CCW(gray, step1) : rotate90CW(gray, step1);
  gray.release();

  // Step 2: additional transform (180, flipH, flipV, or none)
  int subMode = orientMode % 4;
  switch (subMode) {
    case 0: step2 = std::move(step1); break;
    case 1: rotate180(step1, step2); step1.release(); break;
    case 2: flipH(step1, step2); step1.release(); break;
    case 3: flipV(step1, step2); step1.release(); break;
  }

  // Apply keystone correction
  if (keystoneVal != 0.0f) {
    keystoneH(step2, corrected, keystoneVal);
    step2.release();
    output = &corrected;
  }

  // Send raw grayscale bytes with dimensions in headers
...
For the final demo, I disconnected the Hall effect sensor entirely. The system works fine without position tracking — I just have to mentally keep track of which slide I'm on, which defeats some of the purpose but at least lets me demonstrate the core hyperlink navigation.
